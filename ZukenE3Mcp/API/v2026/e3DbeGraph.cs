...
{
    public class e3DbeGraph : IDisposable
    {
        private readonly Zuken.E3.Interop.e3DbeGraph _raw;
        private bool _disposed;

        public e3DbeGraph(Zuken.E3.Interop.e3DbeGraph raw)
        {
            _raw = raw;
        }

        public Zuken.E3.Interop.e3DbeGraph RawObject => _raw;

        public int CreateArc(int id, double x, double y, double radius, double start, double end)
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.CreateArc(id, x, y, radius, start, end));
        }

        public int CreateBlob(int id, string filename)
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.CreateBlob(id, filename));
        }

        public int CreateCircle(int id, double x, double y, double radius)
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.CreateCircle(id, x, y, radius));
        }

        public int CreateCloud(int id, int pnts, ref object x, ref object y)
        {
            object local_x = x;
            object local_y = y;
            var retVal = StaThreadExecutor.Instance.Execute(() => _raw.CreateCloud(id, pnts, ref local_x, ref local_y));
            x = local_x;
            y = local_y;
            return retVal;
        }

        public int CreateCurve(int id, int pnts, ref object x, ref object y)
        {
            object local_x = x;
            object local_y = y;
            var retVal = StaThreadExecutor.Instance.Execute(() => _raw.CreateCurve(id, pnts, ref local_x, ref local_y));
            x = local_x;
            y = local_y;
            return retVal;
        }

        public int CreateImage(int id, double xpos, double ypos, double xsize, double ysize, string filename, int embed)
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.CreateImage(id, xpos, ypos, xsize, ysize, filename, embed));
        }

        public int CreateLine(int id, double x1, double y1, double x2, double y2)
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.CreateLine(id, x1, y1, x2, y2));
        }

        public int CreatePolygon(int id, int pnts, ref object x, ref object y)
        {
            object local_x = x;
            object local_y = y;
            var retVal = StaThreadExecutor.Instance.Execute(() => _raw.CreatePolygon(id, pnts, ref local_x, ref local_y));
            x = local_x;
            y = local_y;
            return retVal;
        }

        public int CreateRectangle(int id, double x1, double y1, double x2, double y2)
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.CreateRectangle(id, x1, y1, x2, y2));
        }
        ...
        public int GetInsideGraphIds(out object ids, int flags)
        {
            object local_ids = default;
            var retVal = StaThreadExecutor.Instance.Execute(() => _raw.GetInsideGraphIds(out local_ids, flags));
            ids = local_ids;
            return retVal;
        }
        ...
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.SaveBlob(filename));
        }
        ...
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.SetBlobInfo(filename));
        }
