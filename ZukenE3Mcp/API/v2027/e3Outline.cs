...
namespace ZukenE3Mcp.API.v2027
{
    public class e3Outline : IDisposable
    {
        ...
        public int CreateDynamicContour(int devi, int type, int pnts, ref object xarr, ref object yarr, double zPosition, double height, int flags)
        {
            object local_xarr = xarr;
            object local_yarr = yarr;
            var retVal = StaThreadExecutor.Instance.Execute(() => _raw.CreateDynamicContour(devi, type, pnts, ref local_xarr, ref local_yarr, zPosition, height, flags));
            xarr = local_xarr;
            yarr = local_yarr;
            return retVal;
        }

        public int CreateDynamicDrillHole(int devi, double x, double y, double zPosition, double height, double radius, int flags)
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.CreateDynamicDrillHole(devi, x, y, zPosition, height, radius, flags));
        }
