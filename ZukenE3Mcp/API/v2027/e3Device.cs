...
namespace ZukenE3Mcp.API.v2027
{
    public class e3Device : IDisposable
    {
        ...
        public int Create2DManufacturingView(int modi, string shtname, string shtformat, ref object devicelist, int position, int before, int shtId, double xmin, double ymin, double xmax, double ymax, double scale, int flags)
        {
            object local_devicelist = devicelist;
            var retVal = StaThreadExecutor.Instance.Execute(() => _raw.Create2DManufacturingView(modi, shtname, shtformat, ref local_devicelist, position, before, shtId, xmin, ymin, xmax, ymax, scale, flags));
            devicelist = local_devicelist;
            return retVal;
        }
        ...
        public int Get2DManufacturingViewIds(out object viewIds, int flags)
        {
            object local_viewIds = default;
            var retVal = StaThreadExecutor.Instance.Execute(() => _raw.Get2DManufacturingViewIds(out local_viewIds, flags));
            viewIds = local_viewIds;
            return retVal;
        }
        ...
        public int GetBomType(out object types, int flags)
        {
            object local_types = default;
            var retVal = StaThreadExecutor.Instance.Execute(() => _raw.GetBomType(out local_types, flags));
            types = local_types;
            return retVal;
        }
        ...
        public int GetRouteCableInSamePath()
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.GetRouteCableInSamePath());
        }
        ...
        public int JumpToPanel3D(int sheetid, int flags)
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.JumpToPanel3D(sheetid, flags));
        }
        ...
        public int SetRouteCableInSamePath(int samepath, int flags)
        {
            return StaThreadExecutor.Instance.Execute(() => _raw.SetRouteCableInSamePath(samepath, flags));
        }
